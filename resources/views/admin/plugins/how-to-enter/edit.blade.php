...
@section('title')
    @unless(empty($editPromoEntry))
        @foreach($editPromoEntry as $promo_entry)
            Edit {{ $promo_entry->name }} - How To Enter
        @endforeach
    @endunless
@endsection
        ...
        @foreach($editPromoEntry as $promo_entry)
            ...
            <div class="panel panel-default primary-container">
                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-md-9">
                            <strong>ID:</strong> {{ $promo_entry->id }} <i class="ti-shift-right-alt"></i> <strong>Created
                                At:</strong> {{ $promo_entry->created_at->format('M d, Y - g:i:s A') }} <i
                                    class="ti-shift-right-alt"></i> <strong>Updated
                                At:</strong> {{ $promo_entry->updated_at->format('M d, Y - g:i:s A') }}
                        </div>

                        <div class="text-right col-md-3">
                            <a href="{{ route('admin.how-to-enter.index') }}"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title" id="myModalLabel">Delete: {{ $promo_entry->name }}</h4>
                        </div>
                        <div class="modal-body">
                            <strong>Are you sure?</strong> This action is irreversible!
                        </div>
                        <div class="modal-footer">
                            <form action="{{ route('admin.how-to-enter.destroy', ['id' => $promo_entry->id]) }}"
            ...
            <div class="panel panel-default primary-container">
                <div class="panel-heading">Edit: {{ $promo_entry->name }} @if($promo_entry->is_active === 1)
                ...
                <div class="panel-body">
                    ...
                    <form action="{{ route('admin.how-to-enter.update', ['id' => $promo_entry->id]) }}" method="post"
                          id="promoTypeEdit">
                        ...
                        <div class="row">
                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }} col-md-6">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                       value="{{ old('name') ?: $promo_entry->name }}"
...
<strong>{{ $errors->first('name') }}</strong>
                            ...
                            <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }} col-md-6">
                                ...
                                <input type="text" class="form-control" id="slug" name="slug"
                                       value="{{ old('slug') ?: $promo_entry->slug }}"
